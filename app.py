...
def render_trade_expander(ticker, team_key, stock_dict):
    ...
    with st.expander(f"💹 거래 입력 & 내역 — 평단 ${avg:.2f} | {shares:.4f}주", expanded=False):
        ...
        trade_type  = fc1.selectbox("구분", ["매수", "매도"], key=f"ttype_{team_key}_{ticker}")
        trade_date  = fc2.date_input("거래일", value=datetime.today(), key=f"tdate_{team_key}_{ticker}")
        trade_price = fc3.number_input("단가 ($)", min_value=0.01, step=0.01, format="%.2f", key=f"tprice_{team_key}_{ticker}")
        trade_qty   = fc4.number_input("수량 (주)", min_value=0.0001, step=0.01, format="%.4f", key=f"tqty_{team_key}_{ticker}")
