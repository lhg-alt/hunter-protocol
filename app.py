"""
HUNTER PROTOCOL v10 — 윌리엄 이평선(MA) 일봉/주봉 전환 완벽 적용판
Streamlit + yfinance | MA 200/240/365 (일봉 & 주봉) 자동 계산 + 하위 스위치
"""
...
def fetch_stock_data(ticker: str):
    try:
        ...
        # 일봉(Daily) 2년 치 데이터 호출
        hist_d = t.history(period="2y", interval="1d")
        # 주봉(Weekly) 10년 치 데이터 호출 (365주 이평선을 위함)
...
with st.expander("🚨 폭락장 절대 원칙 & 분미프 매뉴얼 (클릭하여 숙지)", expanded=False):
    ...
    4. **이동평균선(MA) 타임프레임:** 미래가치 성장주는 일봉(Daily) 기준, 시총이 거대한 초우량주는 주봉(Weekly) 기준으로 상어를 탐지하라.
    5. **10분할 분할매수 절대 원칙:** 저점 도달 시마다 해당 기업 예산의 1/10씩, **주 1회 원칙**으로 최소 3개월간 기계적 매수.
    """)
st.divider()

# ─────────────────────────────────────────
# 거시 경제 레이더 및 트리거 전환 (3-WAY 스위치 + 일/주봉 하위 스위치)
# ─────────────────────────────────────────
...
def process_team_data(team_stocks, team_budget, team_type, is_idx_mode, is_ma_mode, is_weekly_ma):
    invested = 0
    ...
    for s in team_stocks:
        ...
        if data["success"] and data["current"]:
            if is_ma_mode:
                if is_weekly_ma:
                    stage = get_ma_stage(data["current"], data["ma200_w"], data["ma240_w"], data["ma365_w"])
                else:
                    stage = get_ma_stage(data["current"], data["ma200_d"], data["ma240_d"], data["ma365_d"])
            else:
                ath = data["ath"]
                if ath:
                    stock_drop = ((ath - data["current"]) / ath * 100)
                    effective_drop = market_max_drop if is_idx_mode else stock_drop
...
current_white_invested = process_team_data(st.session_state.white_stocks, target_white_budget, "white", is_index_mode, is_ma_mode, is_weekly_ma)
current_blue_invested = process_team_data(st.session_state.blue_stocks, target_blue_budget, "blue", is_index_mode, is_ma_mode, is_weekly_ma)
...
# ─────────────────────────────────────────
# 종목 카드 렌더 함수 (파라미터 명시적 처리)
# ─────────────────────────────────────────
def render_stock_card(ticker, data, stage, effective_val, alloc_budget, alloc_w, team_color, team_key, special_type=None, is_idx_mode=False, is_ma_mode=False, is_weekly_ma=False):
...
def render_team(team_key, team_label, team_budget, team_color, team_emoji, is_idx_mode, is_ma_mode, is_weekly_ma):
    ...
    for i in range(0, max(len(stocks), 1), 2):
        ...
        for col, stock_dict in zip(cols, batch):
            ...
            with col:
                ...
                if is_ma_mode:
                    if is_weekly_ma:
                        ma200, ma240, ma365 = data["ma200_w"], data["ma240_w"], data["ma365_w"]
                    else:
                        ma200, ma240, ma365 = data["ma200_d"], data["ma240_d"], data["ma365_d"]
                    ...
                    effective_val = market_max_drop if is_idx_mode else stock_drop
                ...
                render_stock_card(
                    ...
                    is_idx_mode=is_idx_mode, is_ma_mode=is_ma_mode, is_weekly_ma=is_weekly_ma
                )
...
white_rows = render_team("white", "백팀 (안전금고)", target_white_budget, "#2563eb", "🛡", is_index_mode, is_ma_mode, is_weekly_ma)
st.divider()

st.markdown("### 🚀 청팀 (Blue Team) — 돈을 불려줄 세포분열 공간")
blue_rows = render_team("blue", "청팀 (세포분열)", target_blue_budget, "#10b981", "🚀", is_index_mode, is_ma_mode, is_weekly_ma)
st.divider()
