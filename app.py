"""
HUNTER PROTOCOL v23 — 매수/매도 거래 입력 & 평단가 자동계산 추가
기존 v22 기반 + trades 히스토리 시스템 통합
"""
...
# 거래 계산 유틸
# ─────────────────────────────────────────
def calc_position(trades_list):
    """거래 내역으로 평단가·보유수량·실현손익 계산"""
    shares = 0.0
    total_cost = 0.0
    realized_pnl = 0.0
    ...
    for t in trades_list:
        if t["type"] == "buy":
            ...
            if shares > 0:
                ...
                realized_pnl += (t["price"] - avg) * t["qty"]
                total_cost -= avg * t["qty"]
                shares -= t["qty"]
                if shares < 0.0001:
                    shares = 0.0
                    total_cost = 0.0
    ...
    return round(avg_price, 4), round(shares, 4), round(realized_pnl, 2)
...
<style>
    ...
    /* 매수/매도 버튼 커스텀 */
    .buy-btn > button { background: #eff6ff !important; border: 2px solid #3b82f6 !important; color: #1d4ed8 !important; border-radius: 10px !important; font-weight: 800 !important; }
    .buy-btn > button:hover { background: #dbeafe !important; }
    .sell-btn > button { background: #fef2f2 !important; border: 2px solid #f87171 !important; color: #dc2626 !important; border-radius: 10px !important; font-weight: 800 !important; }
    .sell-btn > button:hover { background: #fee2e2 !important; }
</style>
...
# ★ 거래 내역 딕셔너리 초기화 {"NVDA": [...], "PLTR": [...]}
if "trades" not in st.session_state:
    if saved_data and "trades" in saved_data:
        st.session_state.trades = saved_data["trades"]
    else:
        st.session_state.trades = {}

# 거래 내역으로 평단가/수량 동기화
def sync_positions_from_trades():
    all_stocks = st.session_state.white_stocks + st.session_state.blue_stocks
    for stock in all_stocks:
        tk = stock["ticker"]
        ticker_trades = st.session_state.trades.get(tk, [])
        if ticker_trades:
            avg, shares, _ = calc_position(ticker_trades)
            stock["avg_price"] = avg
            stock["shares"] = shares
        ...
        if uploaded_file is not None:
            if st.button("🔄 이 파일로 완벽 복구하기", use_container_width=True, type="primary"):
                try:
                    ...
                    st.session_state.trades = restored.get("trades", {})
...
def process_team_data(team_stocks, team_budget, team_type):
    ...
    for s in team_stocks:
        tk = s["ticker"]
        actual_invested += (s.get("avg_price", 0.0) * s.get("shares", 0.0))
...
with tc2:
    ...
    st.markdown(
        ...
        f"<div style='color:#1e293b;font-weight:900;font-size:1.1rem;margin-bottom:4px;'>📊 실제 집행 상태 (거래 내역 연동)</div>"
...
# ─────────────────────────────────────────
# ★★★ 거래 입력 expander (종목별 카드 내부용) ★★★
# ─────────────────────────────────────────
def render_trade_expander(ticker, team_key, stock_dict):
    """매수/매도 입력 + 거래 내역 표시"""
    ticker_trades = st.session_state.trades.get(ticker, [])
    avg, shares, realized_pnl = calc_position(ticker_trades)

    with st.expander(f"💹 거래 입력 & 내역 — 평단 ${avg:.2f} | {shares:.4f}주", expanded=False):

        # ── 거래 입력 폼 ──
        st.markdown("<div style='font-weight:800;color:#0f172a;margin-bottom:8px;'>신규 거래 등록</div>", unsafe_allow_html=True)
        fc1, fc2, fc3, fc4 = st.columns([1.2, 1.5, 1.5, 1.5])
        trade_type = fc1.selectbox("구분", ["매수", "매도"], key=f"ttype_{team_key}_{ticker}", label_visibility="collapsed")
        trade_qty  = fc2.number_input("수량 (주)", min_value=0.0001, step=0.01, format="%.4f", key=f"tqty_{team_key}_{ticker}", label_visibility="collapsed")
        trade_price = fc3.number_input("단가 ($)", min_value=0.01, step=0.01, format="%.2f", key=f"tprice_{team_key}_{ticker}", label_visibility="collapsed")
        trade_date  = fc4.date_input("거래일", value=datetime.today(), key=f"tdate_{team_key}_{ticker}", label_visibility="collapsed")

        total_amt = trade_qty * trade_price
        st.markdown(
            f"<div style='font-size:0.85rem;color:#475569;margin:4px 0 10px;'>거래 금액: <b style='color:#0f172a;'>${total_amt:,.2f}</b> &nbsp;|&nbsp; ≈ ₩{total_amt * st.session_state['exchange_rate']:,.0f}</div>",
            unsafe_allow_html=True
        )
        ...
        with btn_col1:
            ...
            if st.button("🔵 매수 등록", key=f"buy_btn_{team_key}_{ticker}", use_container_width=True):
                if trade_type == "매수":
                    _register_trade(ticker, team_key, stock_dict, "buy", trade_qty, trade_price, trade_date)
        with btn_col2:
            st.markdown('<div class="sell-btn">', unsafe_allow_html=True)
            if st.button("🔴 매도 등록", key=f"sell_btn_{team_key}_{ticker}", use_container_width=True):
                if trade_type == "매도":
                    _validate_and_sell(ticker, team_key, stock_dict, trade_qty, trade_price, trade_date, shares)

        # 통합 등록 버튼 (selectbox 기준)
        if st.button(f"✅ {'매수' if trade_type=='매수' else '매도'} 확정", key=f"confirm_trade_{team_key}_{ticker}", type="primary", use_container_width=True):
            if trade_type == "매수":
                _register_trade(ticker, team_key, stock_dict, "buy", trade_qty, trade_price, trade_date)
            else:
                _validate_and_sell(ticker, team_key, stock_dict, trade_qty, trade_price, trade_date, shares)
        ...
        # ── 거래 내역 ──
        if ticker_trades:
            st.markdown(f"<div style='font-weight:800;color:#0f172a;margin-bottom:6px;'>거래 내역 ({len(ticker_trades)}건)</div>", unsafe_allow_html=True)
            
            rows_html = ""
            for i, t in enumerate(reversed(ticker_trades)):
                orig_idx = len(ticker_trades) - 1 - i
                is_buy = t["type"] == "buy"
                type_badge = f"<span style='background:#eff6ff;color:#2563eb;padding:2px 8px;border-radius:12px;font-weight:800;font-size:0.78rem;'>매수</span>" if is_buy else \
                             f"<span style='background:#fef2f2;color:#dc2626;padding:2px 8px;border-radius:12px;font-weight:800;font-size:0.78rem;'>매도</span>"
                total = t["qty"] * t["price"]
                rows_html += (
                    f"<tr>"
                    f"<td style='padding:7px 10px;border-bottom:1px solid #f1f5f9;color:#475569;font-size:0.82rem;'>{t.get('date','—')}</td>"
                    f"<td style='padding:7px 10px;border-bottom:1px solid #f1f5f9;text-align:center;'>{type_badge}</td>"
                    f"<td style='padding:7px 10px;border-bottom:1px solid #f1f5f9;text-align:right;font-weight:700;'>{t['qty']:.4f}주</td>"
                    f"<td style='padding:7px 10px;border-bottom:1px solid #f1f5f9;text-align:right;font-weight:700;'>${t['price']:.2f}</td>"
                    f"<td style='padding:7px 10px;border-bottom:1px solid #f1f5f9;text-align:right;font-weight:800;color:#0f172a;'>${total:,.2f}</td>"
                    f"<td style='padding:7px 10px;border-bottom:1px solid #f1f5f9;text-align:center;'>"
                    f"<button onclick='void(0)' style='background:#fef2f2;border:1px solid #fca5a5;color:#dc2626;border-radius:6px;padding:2px 8px;font-size:0.75rem;cursor:pointer;'>🗑</button>"
                    f"</td>"
                    f"</tr>"
                )

            table_html = f"""
            <div style='background:white;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden;margin-bottom:8px;'>
            <table style='width:100%;border-collapse:collapse;font-size:0.85rem;'>
              <thead><tr style='background:#f8fafc;'>
                <th style='padding:8px 10px;text-align:left;color:#64748b;font-weight:700;font-size:0.78rem;border-bottom:1px solid #e2e8f0;'>날짜</th>
                <th style='padding:8px 10px;text-align:center;color:#64748b;font-weight:700;font-size:0.78rem;border-bottom:1px solid #e2e8f0;'>구분</th>
                <th style='padding:8px 10px;text-align:right;color:#64748b;font-weight:700;font-size:0.78rem;border-bottom:1px solid #e2e8f0;'>수량</th>
                <th style='padding:8px 10px;text-align:right;color:#64748b;font-weight:700;font-size:0.78rem;border-bottom:1px solid #e2e8f0;'>단가</th>
                <th style='padding:8px 10px;text-align:right;color:#64748b;font-weight:700;font-size:0.78rem;border-bottom:1px solid #e2e8f0;'>거래금액</th>
            ...
            # 거래별 삭제 버튼 (Streamlit 버튼으로)
            st.markdown("<div style='font-size:0.8rem;color:#94a3b8;margin-bottom:4px;'>특정 거래 삭제:</div>", unsafe_allow_html=True)
            del_cols = st.columns(min(len(ticker_trades), 5))
            for i, t in enumerate(reversed(ticker_trades)):
                orig_idx = len(ticker_trades) - 1 - i
                if i < 5:
                    is_buy = t["type"] == "buy"
                    lbl = f"{'🔵' if is_buy else '🔴'} {t.get('date','')[:5]} {t['qty']:.2f}주"
                    if del_cols[i].button(lbl, key=f"del_trade_{ticker}_{orig_idx}"):
                        st.session_state.trades[ticker].pop(orig_idx)
                        _resync_stock(ticker, team_key, stock_dict)
                        save_portfolio()
                        st.rerun()

            if realized_pnl != 0:
                pnl_color = "#059669" if realized_pnl > 0 else "#dc2626"
                st.markdown(
                    f"<div style='background:#f8fafc;border-radius:10px;padding:10px 14px;margin-top:8px;display:flex;justify-content:space-between;'>"
                    f"<span style='color:#64748b;font-weight:700;'>실현 손익</span>"
                    f"<span style='color:{pnl_color};font-weight:900;font-size:1.1rem;'>{'+'if realized_pnl>0 else ''}{realized_pnl:,.2f} USD</span>"
                    f"</div>", unsafe_allow_html=True
                )
        else:
            st.markdown("<div style='color:#94a3b8;font-size:0.85rem;text-align:center;padding:12px;'>거래 내역 없음 — 위에서 매수/매도를 등록하세요</div>", unsafe_allow_html=True)


def _register_trade(ticker, team_key, stock_dict, trade_type, qty, price, date):
    if ticker not in st.session_state.trades:
        st.session_state.trades[ticker] = []
    st.session_state.trades[ticker].append({
        "type": trade_type,
        "qty": float(qty),
        "price": float(price),
        "date": str(date)
...
def _validate_and_sell(ticker, team_key, stock_dict, qty, price, date, current_shares):
    if qty > current_shares + 0.0001:
        st.error(f"❌ 보유 수량({current_shares:.4f}주)보다 많이 매도할 수 없습니다.")
        return
    _register_trade(ticker, team_key, stock_dict, "sell", qty, price, date)

def _resync_stock(ticker, team_key, stock_dict):
    """거래 내역으로 stock_dict 평단/수량 업데이트"""
    trades_list = st.session_state.trades.get(ticker, [])
    avg, shares, _ = calc_position(trades_list)
    stock_dict["avg_price"] = avg
    stock_dict["shares"] = shares
    # session_state 내의 해당 종목도 업데이트
    for s in st.session_state[f"{team_key}_stocks"]:
        if s["ticker"] == ticker:
            s["avg_price"] = avg
            s["shares"] = shares
            break
...
def render_stock_card(ticker, data, stage, effective_val, alloc_budget, alloc_w, team_color, team_key, special_type, stock_dict, stock_idx, total_stocks):
    ...
    with st.container():
        ...
        with info_c2:
            ...
            # 평단가/수량은 이제 거래 내역에서 자동 계산됨
            avg_display = f"${my_avg:.2f}" if my_avg > 0 else "—"
            shares_display = f"{my_shares:.4f}주" if my_shares > 0 else "—"
            ...
            st.markdown(
                ...
                f"<div style='color:#475569;font-size:0.85rem;margin-bottom:2px;font-weight:700;'>⚡ 자동 계산 평단가 / 보유 수량</div>"
                f"<div style='font-weight:900;color:#0f172a;font-size:1.35rem;'>{avg_display} <span style='font-size:0.95rem;color:#64748b;font-weight:600;'>({shares_display})</span></div>"
                f"<div style='font-size:0.75rem;color:#10b981;margin-top:4px;font-weight:700;'>✓ 거래 내역 기반 자동 산출</div>"
                f"</div>", unsafe_allow_html=True)
        ...
        # ★ 기존 평단/수량 수동입력 expander → 거래 입력 expander로 교체
        render_trade_expander(ticker, team_key, stock_dict)
...
def render_team(team_key, team_label, team_budget, team_color, team_emoji):
        ...
        for j, stock_dict in enumerate(batch):
            ...
            with cols[j]:
                ...
                summary_rows.append({
                    ...
                    "보유 수량": f"{stock_dict.get('shares', 0):.4f}주" if stock_dict.get('shares', 0) > 0 else "—",
    ...
    if not st.session_state[add_key]:
        ...
        st.markdown(f"<div style='background:white;border-radius:16px;padding:24px;border:2px dashed {team_color}88;margin-bottom:8px;'>", unsafe_allow_html=True)
        st.markdown(f"<div style='font-weight:900;color:{team_color};font-size:1.1rem;margin-bottom:16px;'>{team_emoji} 새 종목 상세 입력</div>", unsafe_allow_html=True)
        
        c1, c2, c3 = st.columns([2, 1, 1])
        new_tk = c1.text_input("티커 (Ticker)", placeholder="예: TSLA, AMZN...", key=f"inp_{team_key}")
        new_avg = c2.number_input("평단가 ($)", min_value=0.0, step=1.0, key=f"avg_new_{team_key}")
        new_sh = c3.number_input("보유 수량 (주)", min_value=0.0, step=1.0, key=f"sh_new_{team_key}")
        ...
        with ac1:
            if st.button("✅ 저장", key=f"confirm_{team_key}", type="primary", use_container_width=True):
                ...
                if tk and tk not in existing:
                    ...
                    st.session_state[f"{team_key}_stocks"].append({
                        "ticker": tk, "type": added_type, 
                        "avg_price": float(new_avg), "shares": float(new_sh)
                    })
                    st.cache_data.clear()
                    save_portfolio()
