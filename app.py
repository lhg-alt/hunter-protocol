...
def render_trade_expander(ticker, team_key, stock_dict):
    ...
    with st.expander(f"💹 거래 입력 & 내역 — 평단 ${avg:.2f} | {shares:.4f}주", expanded=False):
        ...
        trade_date  = fc2.date_input("거래일", value=datetime.today(), key=f"tdate_{team_key}_{ticker}", label_visibility="collapsed")
        ...
        trade_qty   = fc4.number_input("수량 (주)", min_value=0.0001, step=0.01, format="%.4f", key=f"tqty_{team_key}_{ticker}", label_visibility="collapsed")
        ...
        if ticker_trades:
            ...
            for i, (orig_idx, t) in enumerate(sorted_trades):
                if i < 5:
                    is_buy = t["type"] == "buy"
