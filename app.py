...
if all_rows:
    ...
    headers = ["팀", "티커", "종목명", "현재가", "ATH", "하락률", "단계", "매수(USD)", "매수(KRW)"]
    ...
    for i, r in enumerate(all_rows):
        ...
        for key, (emoji, fg, bg, border) in STAGE_STYLE.items():
            if key in stage_val:
                badge_html = (
                    ...
                    "{emoji} {label}</span>"
                ).format(bg=bg, fg=fg, border=border, emoji=emoji, label=stage_val)
                break
        if not badge_html:
            badge_html = (
                "<span style='display:inline-block;padding:4px 10px;border-radius:20px;"
                "background:#f1f5f9;color:#475569;border:1px solid #e2e8f0;"
                "font-weight:700;font-size:0.78rem;'>⏳ 대기 중</span>"
            )
        ...
        rows_html += (
            ...
            "font-weight:900;color:#0f172a;font-size:0.95rem;'>{ticker}</td>"
            "<td style='padding:11px 14px;text-align:left;border-bottom:1px solid #f1f5f9;"
            "color:#334155;font-size:0.85rem;'>{name}</td>"
            "<td style='padding:11px 14px;text-align:center;border-bottom:1px solid #f1f5f9;"
            "font-weight:700;color:#0f172a;'>{cur}</td>"
            "<td style='padding:11px 14px;text-align:center;border-bottom:1px solid #f1f5f9;"
            "color:#475569;'>{ath}</td>"
            "<td style='padding:11px 14px;text-align:center;border-bottom:1px solid #f1f5f9;"
            "font-weight:700;color:{dc};'>{drop}</td>"
            ...
            dc=dc, drop=drop_val,
            stage=badge_html,
            bs=buy_style, buy=buy_val,
            ks=krw_style, krw=buy_krw,
        )

    table_html = """
    <div style='background:white;border-radius:20px;border:1px solid #e2e8f0;
    overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,0.06);margin-bottom:16px;'>
